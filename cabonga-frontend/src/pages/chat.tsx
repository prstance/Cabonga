...
const Chat: Component = () => {
  ...
  const [group] = createResource<chatGroupType, {slug: string; accessToken: () => string}>({
    slug: params.slug,
    accessToken: () => String(authStore.authTokens?.access)
  }, getGroup);
...
const WSCon: Component<{chatRef: HTMLDivElement | undefined; group: groupType; message: Accessor<string>; setMessage: Setter<string>; messages: Accessor<messageType[]>; setMessages: Setter<messageType[]>}> = props => {
  ...
  const websocketUrl = "ws://" + "127.0.0.1:8000" + "/chat/" + group().slug + "/";
